# Agents Store — Gemini CLI Extensions
...
Consumer access to the Gemini CLI closed on 2026-06-18; this format is maintained for **enterprise Gemini Code Assist** (the consumer-facing successor is Antigravity CLI).

The [geminicli.com](https://geminicli.com) gallery — and `gemini extensions install <url>` — only index a repository that carries `gemini-extension.json` at its **root**. This repository is a monorepo where every extension lives in its own subdirectory, so it will not appear in the gallery and will not resolve directly. Install locally instead:

```bash
git clone <this-repo>
gemini extensions link agents-store-gemini-extensions/<plugin-name>
```
